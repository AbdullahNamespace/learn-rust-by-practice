// ════════════════════════════════════════════════════════════════════════════
// ## 🔷 Problem 27: print_range_n_to_1.rs
// ════════════════════════════════════════════════════════════════════════════
// **Category:** Loops & Iteration | الحلقات والتكرار
// **Difficulty:** Easy | سهل
// **Source:** programmingadvices.com
// ════════════════════════════════════════════════════════════════════════════

// ────────────────────────────────────────────────────────────────────────────
// 📝 DESCRIPTION | الوصف
// ────────────────────────────────────────────────────────────────────────────
//
// EN:
// Write a program that reads a positive number N from the user and prints all
// numbers from N down to 1 using three different loop types: while loop, loop
// with break, and for loop.
//
// AR:
// اكتب برنامجاً يقرأ رقماً موجباً N من المستخدم ويطبع جميع الأرقام من N إلى 1
// (تنازلياً) باستخدام ثلاثة أنواع مختلفة من الحلقات: while، loop مع break، و for.
//
// ────────────────────────────────────────────────────────────────────────────

// ────────────────────────────────────────────────────────────────────────────
// 💡 EXAMPLES | الأمثلة
// ────────────────────────────────────────────────────────────────────────────
//
// Example 1:
// Input:  5
// Output: 5, 4, 3, 2, 1 (printed three times using different loops)
//   Why:    Program prints all numbers from 5 down to 1
//
// Example 2:
// Input:  3
// Output: 3, 2, 1 (printed three times using different loops)
//   Why:    Program prints all numbers from 3 down to 1
//
// Example 3 (Edge Case):
// Input:  1
// Output: 1 (printed three times using different loops)
//   Why:    When N=1, only one number is printed
//
// ────────────────────────────────────────────────────────────────────────────

// ────────────────────────────────────────────────────────────────────────────
// ⚠️ CONSTRAINTS | القيود
// ────────────────────────────────────────────────────────────────────────────
//
// • N must be a positive integer (N > 0) | يجب أن يكون N عدداً صحيحاً موجباً
// • Use three different loop types | استخدم ثلاثة أنواع مختلفة من الحلقات
// • Print numbers in descending order | اطبع الأرقام بترتيب تنازلي
//
// ────────────────────────────────────────────────────────────────────────────

// ────────────────────────────────────────────────────────────────────────────
// 🔧 FUNCTION SIGNATURES | توقيعات الدوال
// ────────────────────────────────────────────────────────────────────────────
//
// fn read_number() -> i32
// fn print_range_using_while(n: i32)
// fn print_range_using_loop(n: i32)
// fn print_range_using_for(n: i32)
//
// ────────────────────────────────────────────────────────────────────────────

use std::io::{self, Write};

// ======================
//     INPUT FUNCTIONS
// ======================

fn read_positive_number(prompt: &str) -> i32 {
    loop {
        let input = read_number::<i32>(prompt);

        if input > 0 {
            return input;
        }

        print_error("invalid input, please enter a positive number");
    }
}

// ======================
//     OUTPUT FUNCTIONS
// ======================

fn print_range_using_while(n: i32) {
    println!("Using while loop:");
    if n < 1 {
        return;
    }
    let mut counter = n;
    while counter >= 1 {
        print!("{} ", counter);
        counter -= 1;
    }
    println!();
}

fn print_range_using_loop(n: i32) {
    println!("Using loop:");
    if n < 1 {
        return;
    }
    let mut counter = n;
    loop {
        if counter < 1 {
            break;
        }

        print!("{} ", counter);

        counter -= 1;
    }
    println!();
}

fn print_range_using_for(n: i32) {
    println!("Using for loop:");
    if n < 1 {
        return;
    }

    for i in (1..=n).rev() {
        print!("{} ", i);
    }
    // other
    // for i in 1..=n {
    //     let j = n - i + 1;
    //     print!("{} ", j);
    // }

    println!();
}

fn print_result(n: i32) {
    print_range_using_while(n);
    print_range_using_loop(n);
    print_range_using_for(n);
}

// ======================
//         MAIN
// ======================

fn main() {
    print_result(read_positive_number("Please enter a number"));
}

// ======================
//   UTILITY FUNCTIONS
// ======================

fn read_string(prompt: &str) -> Result<String, io::Error> {
    print!("{}: ", prompt);
    io::stdout().flush()?;

    let mut input = String::new();
    io::stdin().read_line(&mut input)?;

    Ok(input.trim().to_string())
}

fn read_number<T: std::str::FromStr>(prompt: &str) -> T {
    loop {
        let input = match read_string(prompt) {
            Ok(value) => value,
            Err(_) => {
                print_error("invalid input I/O");
                continue;
            }
        };

        match input.trim().parse() {
            Ok(value) => return value,
            Err(_) => {
                print_error("invalid input, please enter a number");
            }
        }
    }
}

fn print_error(prompt: &str) {
    println!("Error: {}!", prompt);
}
